...
window.onload = function () {
    ...
    form.addEventListener("submit", function (event) {
      ...
      cityName = cityName.trim()  // remove leading and trailing spaces;
      cityName = cityName.replace(/\s+/g, " "); // replace multiple spaces with a single space;
      cityName = cityName.replace(/\s/g, "+"); // replace spaces with a plus sign;
      cityName = cityName.replace(/[^a-zA-Z0-9+]/g, ""); // remove special characters;
      cityName = cityName.toLowerCase(); // convert to lowercase;
      // Check if the city name is valid and not empty or null 
      // or undefined or a number or a special character or letter+number
      if (
        cityName === "" ||
        cityName === null || 
        cityName === undefined || 
